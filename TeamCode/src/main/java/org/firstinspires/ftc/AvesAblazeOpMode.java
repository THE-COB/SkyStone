...
public abstract class AvesAblazeOpMode extends LinearOpMode implements AvesAblazeInterface {
    ...
    public void driveByTics (int numTics, boolean isForward, double power){
        ...
        if(isForward){
            if(power<0){
                while(getMotorPosition(0) >= origPos-numTics && opModeIsActive()){
                    upDown(power);
                    telemetry.addData("Condition", Math.abs(getMotorPosition(0)-(origPos+numTics)));
                    telemetry.update();
                }
                
                while (Math.abs(getMotorPosition(0)-(origPos-numTics)) >= 15 && opModeIsActive()){
                    upDown(.05);
